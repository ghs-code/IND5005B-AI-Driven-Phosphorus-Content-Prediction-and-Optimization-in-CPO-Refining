...
`make factor-validation` 当前只读取已有 2024/2025 质量表格，输出会把证据分为 `observed_effect`、`proxy_supported`、`not_assessable`。报告可以确认当前表格中的 history/tank/source/time proxy 是否有信号，但不会把 rainfall、soil、storage、mill origin、metal complexes 等写成已直接确认因素。
...
其中包括 `factor_availability.csv`、`factor_univariate_tests.csv`、`factor_group_model_comparison.csv`、`factor_permutation_importance.csv`、`factor_evidence_matrix.csv`、`proxy_factor_impact_summary.csv`、`proxy_to_hypothesis_mapping.csv`、`factor_recommendations.md` 和 `final_factor_conclusion.md`。feed phosphorus 模型只使用预测时可得的 pre-feed 字段；`rbd_*`、acid/bleaching dosing 和 `p_removal_delta` 只用于 process-response 讨论，不进入 feed target 预测特征，也不用于支持自动 dosing optimization。
